...
-- [回答]
SELECT store_name 店舗名,goods_name 商品名,quantity 在庫数 FROM stock_table 在庫テーブル
JOIN goods_table 商品テーブル
ON 在庫テーブル.goods_code = 商品テーブル.goods_code
JOIN store_table 店舗テーブル
ON 在庫テーブル.store_code = 店舗テーブル.store_code
...
-- [回答]
SELECT store_code 店舗コード FROM stock_table
GROUP BY store_code
HAVING 
